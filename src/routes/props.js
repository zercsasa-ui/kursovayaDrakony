...
router.put('/:id', upload.any(), async (req, res) => {
  try {
    ...
    // Получаем текущий проп, чтобы узнать старый путь к изображению
    const currentProp = await Product.findOne({
      where: { id, type: 'props' }
    });
    ...
    if (req.files && req.files.length > 0) {
      ...
      if (imageFile) {
        ...
        oldImagePath = currentProp.imageUrl;
      }
    }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Получаем проп перед удалением, чтобы узнать путь к изображению
    const prop = await Product.findOne({
      where: { id, type: 'props' }
    });
    ...
    const imageDeletionPromise = deleteImageFile(prop.imageUrl);
